...
# --- Helpers (kit autocontido — utilitários do sistema EE inlinados) ---
def esc(s):
    return "" if s is None else _html.escape(str(s), quote=False)
...
def truncate(s, max_chars=180, suffix="…"):
    ...
    if len(s) <= max_chars:
        return s
    return s[:max_chars].rsplit(" ", 1)[0] + suffix
